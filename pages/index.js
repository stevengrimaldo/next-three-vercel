...
const Material = ({ config, uv }) => {
  ...
  const [texture, texture2, texture3] = useMemo(() => [tex, tex2, tex3], [
    tex,
    tex2,
    tex3,
  ]);
...
const Plane = ({ config }) => {
  ...
  const onHover = ({ clientX, clientY, object, uv: uvu }) => {
    ...
    } else if (intersection.curreent !== intersectedPlane) {
      // first intersection
...
const Gui = ({ data, onUpdate }) => (
  <DatGui data={data} onUpdate={onUpdate}>
    <DatNumber path="amplitude" label="Amplitude" min={0} max={10} />
    <DatNumber
      path="effect"
